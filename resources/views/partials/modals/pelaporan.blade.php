        ...
        <div
            ...
            <div>
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Tambah Data Pelaporan</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">Isi form berikut untuk menambahkan data baru.</p>
                    </div>
                </div>
                <form method="POST" action="/dashboard/pelaporan" enctype="multipart/form-data">
                    @csrf
                    <div class="mt-4">
                        <label class="block font-medium text-gray-700">Lembaga</label>
...
<div x-show="$store.modal.editModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
            <form method="POST" x-ref="editForm" :action="'/dashboard/pelaporan/' + $store.modal.editPelaporan.id"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Edit Data Pelaporan</h3>
                <div class="mt-2">
                    <p class="text-sm text-gray-500">Update the form below to modify the existing data.</p>
                </div>
                <div class="mt-4">
                    <label class="block font-medium text-gray-700">Lembaga</label>
                    <input type="text" name="lembaga" x-model="$store.modal.editPelaporan.lembaga" required
                        class="mt-1 block w-full border-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                </div>
                <div class="mt-4">
                    <label class="block font-medium text-gray-700">Nama</label>
                    <input type="text" name="nama" x-model="$store.modal.editPelaporan.nama"
                        class="mt-1 block w-full border-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                </div>
                <div class="mt-4">
                    <label class="block font-medium text-gray-700">Alamat</label>
                    <input type="text" name="alamat" x-model="$store.modal.editPelaporan.alamat"
                        class="mt-1 block w-full border-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                </div>
                <div class="mt-4">
                    <label class="block font-medium text-gray-700">Keterangan</label>
                    <textarea name="keterangan"
                        class="mt-1 block w-full border-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50"
                        x-model="$store.modal.editPelaporan.keterangan"></textarea>
                </div>
